...
const createObject = (_, index) => {
  ...
  if (previousImageNumbers.length > (OBJECTS_NUMBERS)) {
    throw(new Error('Необходимое количество объектов создано.'));
  }

  let currentImageNumber = getRandomInt(1, OBJECTS_NUMBERS);

  while (previousImageNumbers.includes(currentImageNumber)) {
    currentImageNumber = getRandomInt(1, OBJECTS_NUMBERS);
  }
...
const createObjects = () => Array.from({length: OBJECTS_NUMBERS}, createObject);
